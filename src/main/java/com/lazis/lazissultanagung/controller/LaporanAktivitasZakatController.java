...
public class LaporanAktivitasZakatController {
    ...
    public ResponseEntity<Map<String, Object>> getZakatActivityReport(
        ...
        List<Object[]> results = transactionRepository.getZakatActivityReportNative(bulan1, bulan2);
        ...
        for (Object[] row : results) {
            ...
            if (uraian.startsWith("Penerimaan Dana Zakat ")) {
                Map<String, Double> breakdown = new LinkedHashMap<>();
                breakdown.put(month1Name, val1);
                breakdown.put(month2Name, val2);
                penerimaanDana.put(uraian.replace("Penerimaan Dana ", ""), breakdown);
            } else if (uraian.equals("Jumlah Penerimaan Dana Zakat")) {
                penerimaanDana.put("Total Bulan " + month1Name, val1);
                penerimaanDana.put("Total Bulan " + month2Name, val2);
            } else if (uraian.equals("Fakir") || uraian.equals("Miskin") || uraian.equals("Amil") ||
                    uraian.equals("Muallaf") || uraian.equals("Fiisabilillah") || uraian.equals("Ibnu Sabil")) {
                Map<String, Double> breakdown = new LinkedHashMap<>();
                breakdown.put(month1Name, val1);
                breakdown.put(month2Name, val2);
                pendayagunaanDana.put(uraian, breakdown);
            } else if (uraian.equals("Jumlah Pendayagunaan Dana Zakat")) {
                pendayagunaanDana.put("Total Bulan " + month1Name, val1);
                pendayagunaanDana.put("Total Bulan " + month2Name, val2);
            } else if (uraian.equals("Surplus (Defisit) Dana Zakat")) {
                response.put("Surplus (Defisit) Dana " + month1Name, val1);
                response.put("Surplus (Defisit) Dana " + month2Name, val2);
            } else if (uraian.equals("Saldo Awal Dana Zakat")) {
                response.put("Saldo Awal Dana " + month1Name, val1);
                response.put("Saldo Awal Dana " + month2Name, val2);
            } else if (uraian.equals("Saldo Akhir Dana Zakat")) {
                response.put("Saldo Akhir Dana " + month1Name, val1);
                response.put("Saldo Akhir Dana " + month2Name, val2);

                // Save Saldo Akhir to database
                // saveSaldoAkhir(45L, month1, year1, val1);
                // saveSaldoAkhir(45L, month2, year2, val2);
            }
        }

        Map<String, Object> finalResponse = new LinkedHashMap<>();
        finalResponse.put("Penerimaan Dana", penerimaanDana);
        finalResponse.put("Pendayagunaan Dana", pendayagunaanDana);
