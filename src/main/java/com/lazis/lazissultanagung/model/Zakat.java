...
public class Zakat implements BaseModel {
    ...
    @org.hibernate.annotations.Formula("(SELECT COALESCE(SUM(t.debit), 0) FROM transaction t WHERE t.zakat_id = id AND t.penyaluran = 0 AND t.success = 1)")
    private double amount;

    @org.hibernate.annotations.Formula("(SELECT COALESCE(SUM(d.distribution_amount), 0) FROM distribution d WHERE d.zakat_id = id AND d.success = 1)")
